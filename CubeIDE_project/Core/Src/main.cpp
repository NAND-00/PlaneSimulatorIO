...
/* USER CODE BEGIN PV */
bipolarMotor b_motor1(GPIOG, GPIO_PIN_4, GPIOG, GPIO_PIN_3, GPIOD, GPIO_PIN_14, GPIOG, GPIO_PIN_5, GPIOG, GPIO_PIN_2, GPIOD, GPIO_PIN_15, DAC_CHANNEL_1);
unipolarMotor u_motor1(GPIOB, GPIO_PIN_15, GPIOB, GPIO_PIN_14, GPIOB,GPIO_PIN_13, GPIOB, GPIO_PIN_12);
bipolarMotor b_motor2(GPIOD, GPIO_PIN_12, GPIOD, GPIO_PIN_11, GPIOD, GPIO_PIN_8, GPIOD, GPIO_PIN_13, GPIOD, GPIO_PIN_10, GPIOD, GPIO_PIN_9, DAC_CHANNEL_2);
...
extern vector<unique_ptr<Device> > allDevices;
extern USBD_HandleTypeDef hUsbDeviceHS;
extern int readyToMoveStepper;
/* USER CODE END PV */
...
/* Configure the peripherals common clocks */
  ...
  readyToMoveStepper = 0;
  HAL_DAC_Start(&hdac1, DAC_CHANNEL_1);
  HAL_DAC_Start(&hdac1, DAC_CHANNEL_2);

  activateDevices(allDevices);
  HAL_StatusTypeDef ret = HAL_ADCEx_Calibration_Start(&hadc2, ADC_CALIB_OFFSET, ADC_SINGLE_ENDED);
    if (ret != HAL_OK)
    	  {
    	    Error_Handler();
    	  }
  ...
  while (1)
  {
	  int bufferPosition = 0;
		  ...
		  for(unsigned int i = 0; i<allDevices.size(); i++){
			  allDevices[i]->readData();
			  allDevices[i]->updateDeviceState();
			  allDevices[i]->updateDeviceBuffer();
			  allDevices[i]->returnDeviceBuffer(IN_Buffer, bufferPosition);
			  bufferPosition += allDevices[i]->returnDeviceBufferSize();
		  }

		  USBD_HID_SendReport(&hUsbDeviceHS,IN_Buffer,HID_IN_BUFFER_SIZE);

		  if(readyToMoveStepper == 1){
		  		 readyToMoveStepper = 0;
		  		b_motor1.moveIfRequested();
		  		u_motor1.moveIfRequested();
		  		b_motor2.moveIfRequested();
				u_motor2.moveIfRequested();
		  	  }
    /* USER CODE END WHILE */
...
static void MX_ADC3_Init(void)
{
  ...
  */
  ...
  hadc3.Init.Oversampling.Ratio =  ADC3_OVERSAMPLING_RATIO_32;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : PD8 PD9 PD10 PD11
                           PD12 PD13 PD14 PD15 */
  GPIO_InitStruct.Pin = GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11
  ...
  GPIO_InitStruct.Pin = GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_13;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
